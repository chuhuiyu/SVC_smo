...
def confusion_matrix(y,X_test,model):
    confusion = np.zeros((2,2))
    for ind in range(y.shape[0]):
        pred = decision_function(model.alphas,model.y,linear_kernel,model.X,X_test[ind],model.b)>0
        if (pred == True) & (int(y[ind]) == 1) :
            confusion[0][0] += 1
        elif (pred == True) & (int(y[ind]) == -1):
            confusion[1][0] += 1
        elif (pred  == False) & (int(y[ind]) == 1):
            confusion[0][1] += 1
        elif (pred ==False) & (int(y[ind]) == -1):
            confusion[1][1] += 1
    return confusion
